...
Measured on 100 chunks and the 27 held-out golden questions at `578d22c`.

| Arm | recall@5 | hit@5 | MRR | ndcg@5 |
|-----|---------:|---------:|---------:|---------:|
| `bm25` _(shipped)_ | **0.724** | **1.000** | 0.896 | **0.819** |
| `dense` | 0.562 | 0.889 | 0.708 | 0.587 |
| `hybrid` | 0.598 | 0.963 | 0.790 | 0.656 |
| `rerank` | 0.625 | 0.963 | **0.907** | 0.741 |
| `bm25+rerank` | 0.660 | 0.963 | **0.907** | 0.769 |
...
push to main
   ...
   └─ Backend CI · Retrieval eval
        ├─ run_eval.py --check              (all 71 questions — the gate)
        ├─ run_eval.py --split holdout --publish   (the 27 held-out — the report)
